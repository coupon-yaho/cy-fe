  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "쿠폰 야~호 · 브랜드 데이 선착순 쿠폰" },
      {
        ...
        content: "지금 어느 브랜드가 열려 있는지, 수량이 얼마나 남았는지 봅니다.",
      },
    ...
    <div>
      ...
      <section className="mx-auto w-full max-w-6xl px-5 py-14">
        ...
        <SectionHead
          eyebrow="다가오는 일정"
          title="열리는 순서대로"
          note="브랜드마다 매월 같은 요일, 같은 시각에 한 번 엽니다. 수량이 먼저 떨어지면 마감 전에도 닫습니다."
          action={
      ...
      <div className="relative z-[1] mx-auto grid w-full max-w-6xl items-center gap-5 px-5 pt-8 pb-2 sm:grid-cols-[minmax(0,42%)_minmax(0,1fr)] md:gap-8 md:grid-cols-[minmax(0,44%)_minmax(0,1fr)] md:pt-10 lg:grid-cols-[minmax(0,46%)_minmax(0,1fr)]">
        ...
        <div className="order-1 sm:order-2">
          ...
          <p className="yh-hero-lede yh-lede mt-5 max-w-[40ch] text-white/72">
            수량은 정해져 있고, 먼저 누른 사람이 받습니다. 다 나가면 마감 시각 전에도 닫으니 열리는
            시각을 알아 두는 편이 낫습니다.
          </p>
  ...
  return (
    ...
    <section className="bg-yh-paper-2">
      <div className="mx-auto w-full max-w-6xl px-5 py-14">
        ...
        <SectionHead
          title="브랜드 데이 순서"
          note="지금 잡혀 있는 일정입니다. 브랜드마다 열리는 주와 요일이 다르고, 일정은 바뀌기도 합니다."
        />
...
const STEPS = [
  {
    n: "01",
    head: "입장",
    body: "열리는 시각에 누르면 바로 들어가거나, 사람이 몰리면 줄을 섭니다.",
  },
  {
    n: "02",
    head: "대기",
    body: "순번과 남은 시간은 1초마다 새로 옵니다. 창만 열어 두면 됩니다.",
  },
  {
    n: "03",
    head: "발급",
    body: "입장 토큰을 받은 사람만 발급으로 넘어갑니다. 한 사람 한 장, 수량을 넘겨 나가는 일은 없습니다.",
  },
];
...
const RULES = [
  {
    head: "한 회차에 한 장",
    body: "같은 회차에서는 한 사람이 한 장만 받습니다.",
  },
  {
    head: "수량이 떨어지면 바로 마감",
    body: "마감 시각이 남아 있어도 수량이 떨어지면 그때 닫습니다. 남은 수량은 화면에서 계속 다시 셉니다.",
  },
  {
    head: "정해진 수량만",
    body: "몇 명이 한꺼번에 눌러도 준비된 수량보다 많이 나가지 않습니다.",
  },
];
