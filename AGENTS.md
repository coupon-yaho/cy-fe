...
확정**했습니다. 아래 표의 **잠긴 것**을 수정하는 PR 은 리젝트합니다.

잠그는 단위는 **파일이 아니라 의미**입니다. 파일을 여는 건 자유이고, 그 안에서 무엇을
바꾸느냐가 기준입니다. 지킬 수 없는 규칙은 우회당하고, 한 번 우회되면 나머지 규칙까지
구속력을 잃습니다.

| 파일 | 잠긴 것 | 허용되는 것 |
| --- | --- | --- |
| `src/lib/admin/types.ts` | 필드·타입·enum 값·의미 전부 | 없음 (주석 오탈자 제외) |
| `src/components/admin/state.tsx` | G0-05 표시 규칙 5개의 판정 | 규칙을 바꾸지 않는 렌더 조정 |
| `src/lib/admin/contract.ts` | 엔드포인트 · 요청/응답 타입 · 메서드 의미 | 횡단 관심사 인자 추가 — **optional 이고 하위호환일 때만** (예: CY-369 의 `signal?: AbortSignal`) |
...
**왜 의미 단위인가.** CY-369 는 취소를 구현하려고 `contract.ts` 의 폴링 5개 메서드에
`signal?` 를 붙였습니다. 파일 단위로 잠그면 이건 위반이지만, 데이터 계약은 한 글자도
바뀌지 않았고 `signal` 없이 취소를 구현할 방법도 없습니다. 규칙이 이런 작업을 막으면
다음 세션은 규칙을 무시하는 쪽을 배웁니다.

**판단 기준 한 줄** — 백엔드(A)가 이 변경을 알아야 하면 잠긴 것이고,
몰라도 되면 허용되는 것입니다. `signal` 은 브라우저가 요청을 끊는 일이라 A 와 무관합니다.
필드 하나를 추가하는 건 A 가 내려줘야 하므로 잠긴 것입니다.

**애매하면 고치지 말고 멈추고 물어보십시오.** 특히 값이 안 맞을 때 타입을 넓히는 것은
어떤 경우에도 허용되지 않습니다.
...
## 4. 관리자 API 는 HTTP 전용입니다

`lib/admin/index.ts` 는 `lib/admin/http.ts` 만 연결합니다. `VITE_API_BASE_URL` 이 비어 있으면
같은 출처의 `/api/**` 를 사용하므로 로컬 개발은 Vite 프록시를 거칩니다. 백엔드가 없거나
응답하지 않으면 화면은 `UNAVAILABLE` 로 드러내야 하며, 프론트에서 가짜 응답으로 대체하지
않습니다. 관리자 응답 타입은 실제 백엔드 계약만 표현합니다.
